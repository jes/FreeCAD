...
PyObject* DocumentPy::recompute(PyObject* args)
{
    ...
    PY_TRY
    {
        ...
        int objectCount = 0;
        auto* doc = getDocumentPtr();
        auto& app = App::GetApplication();
        if (app.isAsyncRecomputeEnabled() && useAsync) {
            RecomputeRequest req = {
                .document = doc,
                .callback = nullptr,
            };
            app.queueRecomputeRequest(req);
        } else {
            objectCount = doc->recompute(objs, Base::asBoolean(force), nullptr, options);
        }
